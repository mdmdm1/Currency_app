...
class EditDebtDialog(QDialog):
    def __init__(self, person_name, amount, debt_date, paid_debt, current_debt):
        super().__init__()
        self.setWindowTitle('تعديل الدين')
        ...
        self.debt_date_input.setDate(QDate.currentDate())  # Set the default date to today
        self.debt_date_input.setDisplayFormat("dd-MMM-yy")
        self.paid_debt_input = QLineEdit(paid_debt)
        ...
        form_layout.addLayout(self._create_row(self.debt_date_input, 'تاريخ الدين:'))
        form_layout.addLayout(self._create_row(self.paid_debt_input, 'الدين المدفوع:'))
        form_layout.addLayout(self._create_row(self.current_debt_input, 'الدين الحالي:'))
    ...
    def get_values(self):
        return (
            ...
            self.debt_date_input.text(),
            self.paid_debt_input.text(),
